...
if ( ! function_exists( 'aarambha_real_estate_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function aarambha_real_estate_posted_on() {
...
//				}
//				else {
//					printf( '<span class="tag-links">%1$s</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
//				}
                /* translators: 1: list of tags. */
                printf(
		...
		edit_post_link(
			sprintf(
				...
				/* translators: %s: Name of current post. Only visible to screen readers */
					__( 'Edit <span class="screen-reader-text">%s</span>', 'aarambha-real-estate' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'aarambha_real_estate_post_thumbnail' ) ) :
	/**
	 ...
	 * element when on single views.
	 */
	function aarambha_real_estate_post_thumbnail( $size = 'full', $ratio = '16x9' ) {
			...
			<figure class="featured-image" data-ratio="<?php echo esc_attr( $ratio ); ?>">
				<a class="post-thumbnail" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
					<?php if ( $placeholder_image && $placeholder_image != '' ) : ?>
						<img src="<?php echo esc_url( $placeholder_image ); ?>" alt="<?php esc_attr_e( 'Placeholder Image', 'aarambha-real-estate' ); ?>" />
					<?php else: ?>
						<img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==" alt="<?php esc_attr_e( 'Placeholder Image', 'aarambha-real-estate' ); ?>" />
					<?php endif; ?>
				</a>
			</figure>
		<?php endif;
