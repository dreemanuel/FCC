<?php
/**
 * Customizer Control: aarambha-real-estate-repeater
 *
 ...
 * Repeater control
 */
class Aarambha_Real_Estate_Customize_Repeater_Control extends WP_Customize_Control {
     ...
     * @var string
     */
    public $type = 'aarambha-real-estate-repeater';
    ...
    public function __construct( $manager, $id, $args = array() ) {
        ...
        $this->row_label = array(
            'type' => 'text',
            'value' => esc_attr__( 'row', 'aarambha-real-estate' ),
            'field' => false,
        );
        ...
        if ( empty( $this->button_label ) ) {
            $this->button_label = sprintf( __( 'Add new %s', 'aarambha-real-estate' ), esc_html( $this->row_label['value'] ) );
        }
    ...
    public function enqueue() {
        ...
        wp_enqueue_script( 'aarambha-real-estate-repeater', AARAMBHA_REAL_ESTATE_URI . 'inc/customizer/controls/repeater/assets/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
        wp_enqueue_style( 'aarambha-real-estate-repeater', AARAMBHA_REAL_ESTATE_URI . 'inc/customizer/controls/repeater/assets/repeater.css', null );

        wp_localize_script( 'aarambha-real-estate-repeater', 'aarambha_real_estate_customize',
            array(
                'nonce' => wp_create_nonce( 'aarambha_real_estate_customize_nonce' )
            )
        );
    }
    ...
    protected function render_content() {
        ...
        <?php if ( isset( $this->choices['limit'] ) ) : ?>
            <p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'aarambha-real-estate' ), esc_html( $this->choices['limit'] ) ); ?></p>
        <?php endif; ?>
                ...
                <div class="repeater-row-content">
                    ...
                    <div class="repeater-field repeater-field-{{{ field.type }}}">
                        ...
                        <label>
                            ...
                            <span class="description customize-control-description">{{{ field.description }}}</span>
                            <# } #>
                            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'aarambha-real-estate' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
                        ...
                        <figure class="aarambha-real-estate-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'aarambha-real-estate' ); ?>" >
                            <# if ( field.default ) { #>
                            <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                            <img src="{{{ defaultImageURL }}}">
                            <# } else { #>
                            <?php esc_attr_e( 'No Image Selected', 'aarambha-real-estate' ); ?>
                            <# } #>
                        </figure>

                        <div class="actions">
                            <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'aarambha-real-estate' ); ?></button>
                            <button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'aarambha-real-estate' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'aarambha-real-estate' ); ?>" >
                                <# if ( field.default ) { #>
                                <?php esc_attr_e( 'Change Image', 'aarambha-real-estate' ); ?>
                                <# } else { #>
                                <?php esc_attr_e( 'Add Image', 'aarambha-real-estate' ); ?>
                                <# } #>
                            </button>
                            <# if ( field.default.id ) { #>
                        ...
                        <figure class="aarambha-real-estate-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'aarambha-real-estate' ); ?>" >
                            <# if ( field.default ) { #>
                            ...
                            <# } else { #>
                            <?php esc_attr_e( 'No File Selected', 'aarambha-real-estate' ); ?>
                            <# } #>
                        </figure>

                        <div class="actions">
                            <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                            <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'aarambha-real-estate' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'aarambha-real-estate' ); ?>" >
                                <# if ( field.default ) { #>
                                <?php esc_attr_e( 'Change File', 'aarambha-real-estate' ); ?>
                                <# } else { #>
                                <?php esc_attr_e( 'Add File', 'aarambha-real-estate' ); ?>
                                <# } #>
                            </button>
                            <# if ( field.default.id ) { #>
                            <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
                    ...
                    <# }); #>
                    <button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'aarambha-real-estate' ); ?></button>
                </div>
            </li>
        </script>
        <?php
